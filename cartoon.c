    ...
    while (archivepos < archive->size && !KeyEsc()) {
        ...
        while ((command = cartoondesc_next(cartoondesc)) != NULL && !KeyEsc()) {
            ...
            case 0x08:
                ...
                checkregister(i);
                if (fonts[i] != NULL) {
                    fprintf(stderr, "lord: can not store font resource %d\n",
                            i);
                    exit(1);
                }
            ...
            case 0x10:
                ...
                if (registers[i] == get_command_par(command, registers, 2))
            ...
            case 0x12:
                ...
                pixmaps[i] =
                    pixmap_subscreen(get_command_par(command, registers, 2),
                                     get_command_par(command, registers, 4),
                                     get_command_par(command, registers, 6),
                                     get_command_par(command, registers, 8));
            ...
            case 0x1e:
                ...
                if (return_pos == MAXRESOURCES) {
                    fprintf(stderr, "lord: too much nested cartoon gosubs\n");
                    exit(1);
                }
            ...
            case 0x1f:
                ...
                if (return_pos == 0) {
                    fprintf(stderr, "lord: cartoon return without gosub\n");
                    exit(1);
                }
